...
const OtherProductCard = ({ item }: IFProductCardProps) => {
  return (
    <div
      className={cn(
        "w-full min-w-[90%] md:min-w-[376px] aspect-[376/460] overflow-hidden bg-neutral-50 group transition_common relative"
      )}
    >
      <Image
      ...
      {(item?.author ||
        ...
        <div
          className={cn(
            "absolute left-0 bottom-0 w-full h-[37%] md:max-w-full project-home-blur transition_common "
          )}
        >
          <div className="pr-6 pl-4 pb-4 pt-5 md:pr-14 md:pl-6 md:pt-12 md:pb-6 h-full flex flex-col justify-between gap-4 md:gap-6">
            ...
            <div className="flex flex-col mt-auto">
              <p className="text-text-50 font-bold font-scoutcond text-[26px] md:text-[40px] leading-[100%] line-clamp-1">
                {item?.title}
              </p>
              <p className="text-xs sm:text-base font-normal !leading-[1.4] text-text-200 mt-3 font-inter line-clamp-2">
                {item?.metaDescription}
              </p>
              {/* <p
                className={cn(
